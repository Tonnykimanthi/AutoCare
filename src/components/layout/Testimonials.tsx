...
const Testimonials = () => {
  ...
  return (
    <section className="relative mt-5 p-5 max-sm:px-2">
      ...
      <button
        onClick={prevSlide}
        className="absolute left-5 top-1/2 -translate-y-1/2 rounded-full border border-light p-1 transition duration-300 hover:bg-light hover:text-dark sm:hidden"
      >
        <MdNavigateBefore className="size-7" />
      </button>
      <button
        onClick={nextSlide}
        className="absolute right-5 top-1/2 -translate-y-1/2 rounded-full border border-light p-1 transition duration-300 hover:bg-light hover:text-dark sm:hidden"
      >
